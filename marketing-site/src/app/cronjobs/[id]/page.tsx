...
export default function CronjobDetailPage({ params }: Props) {
  ...
  async function fetchJob() {
    try {
      const { data, error } = await tenantFetch<Cronjob>(`/v1/cronjobs/${params.id}`);
  ...
  async function fetchRuns() {
    try {
      const { data, error } = await tenantFetch<{ items?: CronjobRun[] }>(`/v1/cronjobs/${params.id}/runs`);
  ...
  async function triggerJob() {
    try {
      const { error } = await tenantFetch(`/v1/cronjobs/${params.id}/trigger`, {
        method: 'POST',
      });
      if (error) throw new Error(error);
  ...
  async function deleteJob() {
    ...
    try {
      const { error } = await tenantFetch(`/v1/cronjobs/${params.id}`, {
        method: 'DELETE',
      });
      if (error) throw new Error(error);
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              {job.task_template.codebase_id && (
                <div>
                  <span className="text-sm text-gray-500">Codebase ID</span>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-indigo-600">{job.run_count}</p>
