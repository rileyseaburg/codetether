...
const ZAPIER_INVITE_LINK =
  'https://zapier.com/developer/public-invite/235522/dc2a275ee1ca4688be5a4f18bf214ecb/'
...
const integrationMethods = [
  {
    name: 'Zapier',
    description:
      'Connect CodeTether to 6,000+ apps with no code. 18 components: triggers, actions, searches for tasks, agents, workspaces, cron jobs, billing, and more.',
    icon: ZapierIcon,
    color: 'bg-orange-500',
  ...
  },
  {
    name: 'REST API',
    description:
      'Full programmatic access to create tasks, send messages, and monitor agent activity.',
    icon: TerminalIcon,
    color: 'bg-cyan-500',
  ...
  },
  {
    name: 'GitHub Actions',
    description:
      'AI-powered code review on every pull request. 2-minute setup, no LLM keys needed.',
    icon: TerminalIcon,
    color: 'bg-gray-900',
  ...
  },
  {
    name: 'Webhooks',
    description:
      'Receive real-time notifications when tasks complete, fail, or need attention.',
    icon: WebhookIcon,
    color: 'bg-cyan-500',
...
const quickStartSteps = [
  {
    ...
    description:
      'Click the button below to add CodeTether to your Zapier account.',
    action: {
      label: 'Add to Zapier',
      href: ZAPIER_INVITE_LINK,
      external: true,
  ...
  },
  {
    ...
    description:
      'Use any trigger (Gmail, Slack, etc.) and add CodeTether\'s "Create Task" action.',
    action: {
      label: 'Zapier Dashboard',
  ...
  },
  {
    ...
    description:
      'View task progress and AI responses in your CodeTether dashboard.',
...
export default function GettingStartedPage() {
  const [activeTab, setActiveTab] = useState<'cli' | 'k8s' | 'zapier' | 'api'>(
    'cli',
  )
  const {
    tenantFetch,
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setServerOk(
        healthRes.data?.status === 'ok' || healthRes.data?.status === 'healthy',
      )
    ...
    <div className="space-y-8 pb-12">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Run locally with the CLI, deploy workers to Kubernetes, or integrate
          via Zapier and REST API.
        </p>
      </div>
      ...
      {isAuthenticated && !authLoading && !statusLoading && (
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <span
              className={`inline-block h-2.5 w-2.5 rounded-full ${serverOk ? 'bg-green-500' : serverOk === false ? 'bg-red-500' : 'bg-gray-500'}`}
            />
            <div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <span
              className={`inline-block h-2.5 w-2.5 rounded-full ${workerCount && workerCount > 0 ? 'bg-green-500' : 'bg-yellow-500'}`}
            />
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Workers
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            ...
            <p className="text-sm text-gray-400 mb-4">
              Interactive terminal interface with chat, file tree, and real-time
              tool output.
            </p>
            <pre className="rounded-lg bg-gray-900 p-4 text-sm text-cyan-400 font-mono">
              codetether
            </pre>
          </div>
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            <h3 className="text-lg font-semibold text-white mb-2">
              3. Start an OKR-driven task
            </h3>
            <p className="text-sm text-gray-400 mb-4">
              Type{' '}
              <code className="text-xs bg-gray-700 text-cyan-400 px-1 rounded">
                /go
              </code>{' '}
              with a natural language objective. The agent creates Key Results,
              asks for approval, then executes autonomously.
            </p>
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            <h3 className="text-lg font-semibold text-white mb-2">
              4. Connect to VS Code or Claude Desktop
            </h3>
            <p className="text-sm text-gray-400 mb-4">
              CodeTether exposes 26 tools via the Model Context Protocol. Add
              this config to your project:
            </p>
        ...
        <div className="max-w-3xl space-y-6">
          <div className="rounded-xl border border-purple-500/30 bg-purple-500/5 p-6">
            <div className="flex items-center gap-2 mb-4">
              <span className="text-xs font-semibold px-2 py-1 rounded bg-purple-500/20 text-purple-400">
                ☸️ Kubernetes
              </span>
            ...
            <p className="text-sm text-gray-300 mb-4">
              In Kubernetes mode, CodeTether agents run as persistent{' '}
              <strong className="text-white">worker pods</strong> that connect
              to the A2A server via SSE, register workspaces, and pick up tasks
              automatically. This is different from local mode where you run the
              CLI directly.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            ...
            <p className="text-sm text-gray-400 mb-4">
              Workers connect to the A2A server and poll for tasks. Deploy using
              the helper script or Helm chart.
            </p>
            <div className="space-y-4">
              <div>
                <span className="text-xs font-medium text-gray-400 uppercase tracking-wider">
  ...
  worker --codebases /workspace`}
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            ...
            <p className="text-sm text-gray-400 mb-4">
              ...
              The server routes tasks to workers based on which workspaces they
              manage.
            </p>
...
codetether worker \\
  ...
  --auto-approve safe`}
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="rounded bg-gray-700/50 px-3 py-2">
                ...
                <p className="text-gray-400 mt-0.5">
                  Workspace paths (legacy flag name)
                </p>
              </div>
              <div className="rounded bg-gray-700/50 px-3 py-2">
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            ...
            <p className="text-sm text-gray-400 mb-4">
              Workers POST heartbeats to the server. View active workers, their
              workspaces, and available models in the dashboard.
            </p>
          ...
          <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6">
            <h3 className="text-lg font-semibold text-white mb-4">
              Local vs Kubernetes at a Glance
            </h3>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="text-gray-300">
                  {[
                    {
                      label: 'How you start',
                      local: 'codetether (TUI)',
                      k8s: 'codetether worker',
                    },
                    {
                      label: 'Runs as',
                      local: 'Interactive process',
                      k8s: 'Background pod/service',
                    },
                    {
                      label: 'Task source',
                      local: 'Direct CLI or /go',
                      k8s: 'SSE from A2A server',
                    },
                    {
                      label: 'Workspaces',
                      local: 'Current directory',
                      k8s: 'Registered via --workspace-paths (--codebases legacy)',
                    },
                    {
                      label: 'Swarm agents',
                      local: 'Threads (same process)',
                      k8s: 'Isolated pods',
                    },
                    {
                      label: 'Health checks',
                    ...
                    <tr key={row.label} className="border-b border-gray-800">
                      ...
                      <td className="py-2 text-purple-400 font-mono text-xs">
                        {row.k8s}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-2xl bg-gradient-to-r from-orange-500 to-orange-600 p-8 text-white">
            ...
            <p className="text-lg text-orange-100 mb-6 max-w-2xl">
              The fastest way to automate CodeTether. Connect to 6,000+ apps and
              start triggering AI tasks from emails, Slack, forms, and more — no
              code required.
            </p>
            <a
              href={ZAPIER_INVITE_LINK}
          ...
          <div className="rounded-xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-200 dark:ring-gray-700 p-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Examples of what you can build with CodeTether + Zapier
            </p>
