...
export function ChatInput({ draftMessage, loading, actionStatus, hasSession, onDraftChange, onSend }: ChatInputProps) {
    if (!hasSession) {
        return (
            <div className="border-t border-gray-200 dark:border-gray-700 p-4" role="status">
    ...
    return (
        <footer className="border-t border-gray-200 dark:border-gray-700 p-4" aria-label="Message input">
            <form
            ...
            >
                <div className="flex gap-2 items-end">
                    ...
                    <button
                        type="submit"
                        disabled={!canSend}
                        className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
