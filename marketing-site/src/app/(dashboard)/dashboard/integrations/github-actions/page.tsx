...
const SETUP_STEPS = [
  {
    ...
    description:
      "Generate a CodeTether API token from Settings. You'll add this as a GitHub secret.",
  },
  {
    step: 2,
    title: 'Add secret to repo',
    description:
      'Go to your repo → Settings → Secrets → New secret. Name it CODETETHER_TOKEN.',
  },
  {
    step: 3,
    title: 'Add workflow file',
    description:
      'Copy the workflow below into .github/workflows/codetether-review.yml',
  },
  {
    step: 4,
    title: 'Open a PR',
    description:
      'Every new pull request will automatically get an AI-powered code review.',
  },
]
...
export default function GitHubActionsPage() {
  ...
  const [apiKeys, setApiKeys] = useState<
    Array<{ key: string; name: string; created_at: string }>
  >([])
  ...
  const fetchApiKeys = useCallback(async () => {
    ...
    try {
      const res = await tenantFetch<{
        keys?: Array<{ key: string; name: string; created_at: string }>
      }>('/v1/agent/api-keys')
      ...
      {mode !== 'full' && (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
            Add extra instructions to focus the review. E.g. &quot;Focus on
            security&quot; or &quot;Check for SQL injection&quot;.
          </p>
          <input
            type="text"
            value={extraPrompt}
