...
function chooseRecommendedModel(modelRefs: string[]): string | null {
  ...
  for (const provider of providerPreference) {
    const found = modelRefs.find((ref) =>
      ref.toLowerCase().startsWith(`${provider}:`),
    )
...
function WorkerTypeBadge({ worker }: { worker: Worker }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold rounded-full ${meta.color}`}
    >
      <span>{meta.icon}</span>
      {meta.label}
    </span>
...
function StatusBadge({ lastSeen }: { lastSeen: string }) {
  const elapsed = new Date().getTime() - new Date(lastSeen).getTime()
  const isOnline = elapsed < 120000 // <2 min
  const isStale = elapsed < 24 * 60 * 60 * 1000 // <24h
  const color = isOnline
    ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'
    : isStale
      ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400'
      : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
  const label = isOnline ? 'Online' : isStale ? 'Stale' : 'Offline'
  return (
...
function WorkerCard({
  ...
  const linkedWorkspaces = workspaces.filter(
    (ws) =>
      (worker.codebases || []).includes(ws.id) ||
      ws.worker_id === worker.worker_id,
  )
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <span className="px-2 py-1 text-xs bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400 rounded">
          {modelCount} models
        </span>
        <span className="px-2 py-1 text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 rounded">
          {linkedWorkspaces.length} workspaces
        </span>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">
                {detectWorkerType(worker) === 'codetether-agent'
                  ? 'Rust-based agent with ralph/swarm/rlm capabilities'
                  : 'Python-based worker with agent/build/deploy capabilities'}
              </span>
            </div>
          </div>
...
export default function WorkersPage() {
  ...
  const [chatByWorker, setChatByWorker] = useState<
    Record<string, ChatMessage[]>
  >({})
  const [pendingTaskByWorker, setPendingTaskByWorker] = useState<
    Record<string, string | null>
  >({})
  const [showOffline, setShowOffline] = useState(false)
  const [selectedModelByWorker, setSelectedModelByWorker] = useState<
  ...
  const appendMessage = useCallback(
    (workerId: string, message: ChatMessage) => {
  ...
  const fetchTaskFinalResult = useCallback(
    async (
      ...
      const { data, error: requestError } = await tenantFetch<any>(
  ...
  const startTaskStream = useCallback(
    async (workerId: string, taskId: string, assistantMessageId: string) => {
      closeActiveStream()

      // Handle relative API URLs by resolving against window.location
      const absoluteApiUrl = resolvedApiUrl.startsWith('/')
        ? `${window.location.origin}${resolvedApiUrl}`
        : resolvedApiUrl
      ...
      source.onerror = async () => {
        ...
        patchMessage(workerId, assistantMessageId, (current) => {
          ...
          return {
            ...
            content:
              final.error ||
              'Live stream disconnected before any response was received.',
            status: 'error',
          }
        })
        clearPendingForWorker(workerId)
    ...
    [
      clearPendingForWorker,
      closeActiveStream,
      fetchTaskFinalResult,
      patchMessage,
      resolvedApiUrl,
    ],
  )
    ...
    try {
      const [workersRes, workspacesRes, connectedRes] = await Promise.all([
        tenantFetch<Worker[]>(
          showOffline
            ? '/v1/agent/workers'
            : '/v1/agent/workers?exclude_offline_hours=24',
        ),
        tenantFetch<
          Workspace[] | { workspaces?: Workspace[]; codebases?: Workspace[] }
        >('/v1/agent/workspaces/list'),
        tenantFetch<ConnectedWorkersResponse>('/v1/worker/connected'),
      ])

      if (workersRes.error || workspacesRes.error) {
        throw new Error(
          workersRes.error || workspacesRes.error || 'Failed to fetch data',
        )
      }
      ...
      const workspacesPayload = workspacesRes.data as
        | Workspace[]
        | { workspaces?: Workspace[]; codebases?: Workspace[] }
        | undefined
  ...
  const sendToWorker = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      const selectedModelRef =
        ...
        chooseRecommendedModel(availableModels)
      ...
      const payload = {
        ...
        ...(selectedModelRef
          ? {
              ...
              model: toProviderModel(selectedModelRef),
            }
          : {}),
        metadata: {
      ...
      const { data, error: requestError } = await tenantFetch<{ id?: string }>(
        '/v1/agent/tasks',
        {
          method: 'POST',
      ...
      setPendingTaskByWorker((prev) => ({
        ...prev,
        [chatWorkerId]: data.id || null,
      }))
  ...
  const activeWorkerModelRefs = useMemo(
    () => (activeWorker ? uniqueWorkerModelRefs(activeWorker) : []),
    [activeWorker],
  )
  const recommendedActiveWorkerModel = useMemo(
    () => chooseRecommendedModel(activeWorkerModelRefs),
    [activeWorkerModelRefs],
  )
  const selectedActiveModel =
    chatWorkerId && selectedModelByWorker[chatWorkerId]
      ? selectedModelByWorker[chatWorkerId]
      : recommendedActiveWorkerModel || ''
  const activeChatMessages = chatWorkerId
    ? chatByWorker[chatWorkerId] || []
    : []
  const activeTaskPending = Boolean(
    chatWorkerId && pendingTaskByWorker[chatWorkerId],
  )
  ...
  useEffect(() => {
    ...
    setSelectedModelByWorker((prev) => {
      ...
      const fallback = chooseRecommendedModel(
        uniqueWorkerModelRefs(activeWorker),
      )
  ...
  const onlineWorkers = workers.filter(
    (w) => new Date().getTime() - new Date(w.last_seen).getTime() < 120000,
  )
  ...
  const visibleWorkers = showOffline
    ? workers
    : workers.filter((w) => {
        if (w.is_sse_connected) return true
        return (
          new Date().getTime() - new Date(w.last_seen).getTime() <
          STALE_THRESHOLD_MS
        )
      })
  const deadWorkers = workers.filter((w) => {
    const elapsed = new Date().getTime() - new Date(w.last_seen).getTime()
    return !w.is_sse_connected && elapsed >= OFFLINE_THRESHOLD_MS
  })
  const codetetherWorkers = visibleWorkers.filter(
    (w) => detectWorkerType(w) === 'codetether-agent',
  )
  const agentWorkers = visibleWorkers.filter(
    (w) => detectWorkerType(w) === 'agent',
  )
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Manage and monitor registered workers
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            {showOffline
              ? `Hide Offline (${deadWorkers.length} dead)`
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-yellow-200 dark:border-yellow-800">
          <div className="text-2xl font-bold text-yellow-600">
            {staleWorkers.filter((w) => !deadWorkers.includes(w)).length}
          </div>
      ...
      {loading && visibleWorkers.length === 0 ? (
        ...
        <div className="text-center py-12 text-gray-500">
          ...
          >
            Show all {workers.length} workers (including offline)
          </button>
        </div>
      ) : (
              ...
              <div className="mb-2">
                <ModelSelector
                  ...
                  disabled={
                    ...
                    activeTaskPending ||
                    activeWorkerModelRefs.length === 0
                  }
                />
                {recommendedActiveWorkerModel ? (
                ...
                {!activeWorkerModelRefs.length ? (
                  <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    No advertised models for this worker. Requests will use
                    worker default.
                  </p>
                ) : null}
              </div>
