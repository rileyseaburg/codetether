...
export default function WorkersPage() {
    ...
    const fetchData = useCallback(async () => {
        try {
            const [workersRes, workspacesRes, connectedRes] = await Promise.all([
                tenantFetch<Worker[]>(showOffline ? '/v1/agent/workers' : '/v1/agent/workers?exclude_offline_hours=24'),
    ...
    const visibleWorkers = showOffline
        ...
        : workers.filter(w => {
            ...
            return (new Date().getTime() - new Date(w.last_seen).getTime()) < STALE_THRESHOLD_MS
        })
    ...
    const codetetherWorkers = visibleWorkers.filter(w => detectWorkerType(w) === 'codetether-agent')
    const agentWorkers = visibleWorkers.filter(w => detectWorkerType(w) === 'agent')
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-3 md:grid-cols-5 gap-4 mb-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-yellow-200 dark:border-yellow-800">
                    <div className="text-2xl font-bold text-yellow-600">{staleWorkers.filter(w => !deadWorkers.includes(w)).length}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Stale (&lt;24h)</div>
                </div>
                <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-red-200 dark:border-red-800">
                    <div className="text-2xl font-bold text-red-600">{deadWorkers.length}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Offline (&gt;24h)</div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-indigo-200 dark:border-indigo-800">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">🐍 Workers</div>
                </div>
            </div>
