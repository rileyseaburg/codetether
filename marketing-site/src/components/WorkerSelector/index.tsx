...
function formatWorkerLabel(worker: Worker): string {
  ...
  const runtimeLabel =
    ...
    (worker.worker_runtime === 'rust'
      ? 'Rust Worker'
      : 'CodeTether Python Worker')
  ...
  useEffect(() => {
    ...
    const fetchWorkers = async () => {
      try {
        ...
        const baseWorkers = Array.isArray(workersResponse.data)
...
export function WorkerSelector({
  ...
  if (loading) {
    return (
      <div
        className={`px-3 py-2 text-sm text-gray-500 dark:text-gray-400 ${className}`}
      >
        Loading workers...
      </div>
    )
  ...
  if (workers.length === 0) {
    return (
      <div
        className={`px-3 py-2 text-sm text-yellow-600 dark:text-yellow-400 ${className}`}
      >
  ...
  if (visibleWorkers.length === 0) {
    return (
      <div
        className={`px-3 py-2 text-sm text-orange-600 dark:text-orange-400 ${className}`}
      >
