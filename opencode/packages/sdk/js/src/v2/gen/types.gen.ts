...
export type SessionStatus =
  ...
  | {
      type: "rlm"
      phase: "started" | "analyzing" | "summarizing" | "completing"
      detail?: string
      traceId?: string
      iteration?: number
      subcalls?: number
...
export type EventRlmTraceStart = {
  type: "rlm.trace.start"
  properties: {
    traceId: string
    sessionID: string
    toolId: string
...
export type EventRlmTraceStep = {
  type: "rlm.trace.step"
  properties: {
    traceId: string
    stepType: "exploration" | "iteration" | "code_exec" | "subcall" | "final"
    iteration?: number
    data: {
      [key: string]: unknown
    }
...
export type EventRlmRoutingDecision = {
  type: "rlm.routing.decision"
  properties: {
    tool: string
    sessionID: string
    callID?: string
    decision: "routed" | "passthrough"
    reason: string
...
export type EventRlmSubcall = {
  type: "rlm.subcall"
  properties: {
    sessionID: string
    subcallNumber: number
    promptTokens: number
    responseTokens: number
  }
}

export type EventRlmIteration = {
  type: "rlm.iteration"
  properties: {
    sessionID: string
    iteration: number
...
export type EventRlmProgress = {
  type: "rlm.progress"
  properties: {
    sessionID: string
    phase: "analyzing" | "summarizing" | "completing"
    detail?: string
  }
}

export type EventRlmCompleted = {
  type: "rlm.completed"
  properties: {
    sessionID: string
    inputTokens: number
...
export type KeybindsConfig = {
  ...
  /**
   * RLM compress the session (recursive language model)
   */
  session_rlm?: string
  /**
...
export type Config = {
  ...
  /**
   * RLM (Recursive Language Model) configuration for handling large context
   */
  rlm?: {
    /**
    ...
    mode?: "auto" | "off" | "always"
    /**
     * Context threshold (0.0-1.0) relative to model context window. Default: 0.35
     */
    threshold?: number
    /**
     * Model for root RLM reasoning (provider:model format). Uses subcall_model if not set.
     */
    root_model?: string
    /**
     * Model for RLM subcalls in provider:model format. Default: zai-coding-plan:glm-4.7
     */
    subcall_model?: string
    /**
     * REPL runtime: 'bun' (fastest, default), 'python', or 'rust' (requires evcxr).
     */
    runtime?: "python" | "rust" | "bun"
    /**
     * Maximum REPL iterations before forcing completion. Default: 15
     */
    max_iterations?: number
    /**
     * Maximum sub-LM calls per RLM session. Default: 50
     */
    max_subcalls?: number
  }
  experimental?: {
...
export type SessionRlmData = {
  ...
  url: "/session/{sessionID}/rlm"
}

export type SessionRlmErrors = {
  /**
   * Bad request
   */
  400: BadRequestError
...
export type SessionRlmError = SessionRlmErrors[keyof SessionRlmErrors]

export type SessionRlmResponses = {
  /**
   * RLM compression initiated
   */
  200: boolean
}

export type SessionRlmResponse = SessionRlmResponses[keyof SessionRlmResponses]

export type SessionRlmTraceData = {
  body?: never
  path: {
    /**
     * Session ID
  ...
  url: "/session/{sessionID}/rlm-trace"
}

export type SessionRlmTraceErrors = {
  /**
   * Bad request
   */
  400: BadRequestError
...
export type SessionRlmTraceError = SessionRlmTraceErrors[keyof SessionRlmTraceErrors]

export type SessionRlmTraceResponses = {
  /**
   * RLM trace information
   */
  200: {
    traces: Array<{
      id: string
...
export type SessionRlmTraceResponse = SessionRlmTraceResponses[keyof SessionRlmTraceResponses]
