...
from a2a_server.policy import _build_input, _evaluate_local, check_policy
...
class TestProvenanceVerifier:
    ...
    def test_origin_mismatch_fails(self):
        resource = {
            'ap_session_state': {'origin_intent_hash': 'sha384:original'}
        }
        decision = verify_provenance(
            complete_provenance(), 'tasks:write', resource
        )
    ...
    def test_taint_stripping_fails(self):
        ...
        decision = verify_provenance(
            complete_provenance(ap_inputs=[]), 'tasks:write', resource
        )
        assert not decision.allowed_by_provenance
    ...
    def test_child_cannot_remove_parent_budget_limit(self):
        ...
        assert not decision.allowed_by_provenance
        assert (
            'delegation budget not attenuated: max_tool_calls'
    ...
    def test_partial_provenance_denied_for_sensitive_action(self):
        ...
        assert not decision.allowed_by_provenance
        assert (
            'partial provenance not permitted for sensitive action'
            in decision.failures
        )
...
class TestPolicyIntegration:
    ...
    def test_opa_input_contains_provenance_claims(self):
        ...
        assert (
            inp['input']['provenance']['ap_origin']['intent_hash']
            == 'sha384:origin'
        )
...
class TestProvenanceReviewFeedback:
    def test_partial_provenance_allowed_for_non_sensitive_action(self):
        ...
        assert decision.allowed_by_provenance
        assert decision.partial
        assert set(decision.missing_dimensions) == {
            'origin',
            'delegation',
            'runtime',
            'output',
        ...
        provenance = complete_provenance(
            ap_delegation={
                'chain': [
                    ...
                    {
                        'principal': 'agent:child',
                        'capability': {'operations': ['tasks:read']},
                    },
                ]
            }
        )
    ...
    def test_malformed_origin_missing_intent_hash_is_partial(self):
        ...
        decision = verify_provenance(
            complete_provenance(
                ap_origin={'session_started_at': '2026-04-20T14:00:00Z'}
            ),
            'tasks:write',
        )
        assert decision.allowed_by_provenance
