...
def test_parse_spiffe_id_with_path():
    td, path = spiffe_auth.parse_spiffe_id(
        "spiffe://codetether.run/tenant/acme/agent/orch"
    )
    assert td == "codetether.run"
    assert path == "/tenant/acme/agent/orch"


def test_parse_spiffe_id_root():
    td, path = spiffe_auth.parse_spiffe_id("spiffe://codetether.run")
    assert td == "codetether.run"
    assert path == "/"
...
def test_parse_spiffe_id_rejects_non_spiffe():
    with pytest.raises(ValueError):
        spiffe_auth.parse_spiffe_id("https://codetether.run/x")
...
def test_identity_tenant_and_role():
    ident = spiffe_auth.SpiffeIdentity(
        "spiffe://codetether.run/tenant/acme/agent/orch",
        "codetether.run", "/tenant/acme/agent/orch", {},
    )
...
def test_identity_worker_role_no_tenant():
    ident = spiffe_auth.SpiffeIdentity(
        "spiffe://codetether.run/worker/pool",
        "codetether.run", "/worker/pool", {},
    )
...
def test_to_policy_user_default_role():
    ...
    assert user["spiffe_id"] == "spiffe://codetether.run/tenant/acme/agent/orch"
...
def test_validate_valid_svid():
    token = _make_svid("spiffe://codetether.run/tenant/acme/agent/orch")
    ident = spiffe_auth.validate_jwt_svid(token)
    assert ident.spiffe_id == "spiffe://codetether.run/tenant/acme/agent/orch"
    assert ident.tenant == "acme"


def test_validate_rejects_expired():
    token = _make_svid("spiffe://codetether.run/x", exp_delta=-10)
...
def test_validate_rejects_wrong_audience():
    token = _make_svid("spiffe://codetether.run/x", aud="other-service")
    with pytest.raises(Exception) as exc:
        spiffe_auth.validate_jwt_svid(token)
